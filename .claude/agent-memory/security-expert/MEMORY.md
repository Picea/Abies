...
- [State-retention features need a sensitivity marker](state-retention-features-need-sensitivity-marker.md) — need BOTH a message-side marker (redacts `Cause`) and a model-side `Scrub` (redacts retained snapshots); neither subsumes the other.
