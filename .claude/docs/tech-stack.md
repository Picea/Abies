...
- **CI provider:** GitHub Actions. **Verified** — 17 workflows in `.github/workflows/`: `benchmark.yml`, `cd.yml`, `codeql.yml`, `docs-snippets.yml`, `e2e.yml`, `pr-conduit-integration.yml`, `pr-validation.yml`, `release.yml`, `secrets-scan.yml`, `semgrep.yml`, `squad-heartbeat.yml`, `squad-issue-assign.yml`, `squad-triage.yml`, `sync-squad-labels.yml`, `template-security.yml`, `trivy.yml`, `visual-regression.yml`, `zap-baseline.yml`, `zap-nightly.yml`.
- **Squad-aware automation:** this repo runs its own GitHub Actions integration with the squad framework (`squad-heartbeat.yml`, `squad-issue-assign.yml`, `squad-triage.yml`, `sync-squad-labels.yml`) — not present in vanilla squad-template. **Verified to exist; contents/compatibility with the new `.claude/`-based agent names not audited in this pass** — worth a DevOps follow-up to confirm these workflows reference the current subagent names (`csharp-dev`, not `csharpdev`, etc.) rather than the old squad labels (`squad:csharpdev`).
