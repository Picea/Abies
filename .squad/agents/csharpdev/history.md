...
## Learnings
- 2026-05-06: Migration option 1 implementation: keep direct `Picea` `1.0.0` for non-Glauca projects, and pin only Glauca-coupled projects (`Conduit.Api`, `Conduit.Api.Tests`, `Conduit.ReadStore.PostgreSQL`, `Conduit.ReadStore.PostgreSQL.Tests`) to direct `Picea` `1.0.27-rc-0002` to satisfy current `Picea.Glauca` floor (`>= 1.0.27-rc-0002`) and clear `NU1605` downgrade errors.
- 2026-05-06: Conduit migration to `Picea` `1.0.0` is blocked for Glauca-coupled projects. NuGet currently exposes `Picea.Glauca` `0.1.12`-`0.1.14`, and each version depends on prerelease `Picea` (`>= 1.0.22-rc-0001` or `>= 1.0.27-rc-0002`), which triggers `NU1605` when direct `Picea` is pinned to `1.0.0` in `Picea.Abies.Conduit.Api`, `Picea.Abies.Conduit.Api.Tests`, and `Picea.Abies.Conduit.ReadStore.PostgreSQL.Tests`.
