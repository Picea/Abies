...
public interface INativeBackend<T> where T : class
{
    ...
    /// <summary>Appends a child to the parent's child collection (or content slot).</summary>
    void AppendChild(T parent, string parentTag, T child);

    /// <summary>Inserts a child before an existing child.</summary>
    void InsertBefore(T parent, string parentTag, T child, T before);
