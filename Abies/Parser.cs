...
class Program
{
    ...
    {
        // Example: Parsing "blog/123"
        var blogParser = Parse.S("blog").Slash(Parse.Int());

        var input1 = "blog/123".AsSpan();
        var result1 = blogParser.Parse(input1);
        Console.WriteLine(result1.Success
            ? $"Parsed value: {result1.Value}, Remaining: \"{result1.Remaining.ToString()}\""
            : "Parsing failed.");

        // Example: Parsing "search/wolf"
        var searchParser = Parse.S("search").Slash(Parse.S("wolf"));

        var input2 = "search/wolf".AsSpan();
        var result2 = searchParser.Parse(input2);
        Console.WriteLine(result2.Success
            ? $"Parsed value: {result2.Value}, Remaining: \"{result2.Remaining.ToString()}\""
            : "Parsing failed.");

        // Example: Using LINQ syntax
        var combinedParser =
            from a in Parse.Char('A')
            from digit in Parse.Satisfy(char.IsDigit)
            select (a, digit);

        var input3 = "A5".AsSpan();
        var result3 = combinedParser.Parse(input3);
        Console.WriteLine(result3.Success
            ? $"Parsed tuple: ({result3.Value.a}, {result3.Value.digit}), Remaining: \"{result3.Remaining.ToString()}\""
            : "Parsing failed.");
    }
}
*/
