...
public sealed class FakeBackend : INativeBackend<FakeControl>
{
    ...
    public void InsertBefore(FakeControl parent, string parentTag, FakeControl child, FakeControl before)
    {
        parent.Children.Insert(parent.Children.IndexOf(before), child);
        Log.Add($"insert {child.Id} before {before.Id} in {parent.Id}");
    }
