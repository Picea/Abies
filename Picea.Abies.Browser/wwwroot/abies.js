...
async function initializeDebugger() {
    ...
    try {
        const mod = await loadDebuggerModule();
...
function remountDebuggerAfterRootPatch() {
    ...
    Promise.resolve()
        .then(() => loadDebuggerModule(debuggerModuleUrl))
        .then(mod => {
            if (mod && typeof mod.mountDebugger === "function") {
                mod.mountDebugger();
            }
...
async function loadDebuggerModule(preferredModuleUrl = null) {
    const candidateUrls = [];

    if (typeof preferredModuleUrl === "string" && preferredModuleUrl.length > 0) {
        candidateUrls.push(preferredModuleUrl);
    }

    candidateUrls.push(new URL("./debugger.js", import.meta.url).href);
    candidateUrls.push(new URL("/debugger.js", window.location.href).href);

    const uniqueCandidateUrls = [];
    for (const url of candidateUrls) {
        if (!uniqueCandidateUrls.includes(url)) {
            uniqueCandidateUrls.push(url);
        }
    }

    let lastError = null;

    for (const moduleUrl of uniqueCandidateUrls) {
        try {
            const mod = await import(/* webpackIgnore: true */ moduleUrl);
            debuggerModuleUrl = moduleUrl;
            return mod;
        } catch (err) {
            lastError = err;
        }
    ...
    throw lastError ?? new Error("Debugger module unavailable.");
}
