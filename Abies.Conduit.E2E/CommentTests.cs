...
public class CommentTests : PlaywrightFixture
{
    ...
    public async Task CommentSection_NotLoggedIn_ShowsLoginPrompt()
    {
        ...
        if (await articleLink.CountAsync() > 0)
        {
            ...
            var commentSection = Page.GetByTestId("article-page");
            await Expect(commentSection.GetByText("Sign in")).ToBeVisibleAsync();
            await Expect(commentSection.GetByText("sign up")).ToBeVisibleAsync();
