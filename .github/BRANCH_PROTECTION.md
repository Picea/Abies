...
> **Note**: `CD / build` runs on `push` to `main` and is not a PR status check.
...
| **Status Checks** | PR Validation, CodeQL | Automated PR quality gates |
...
4. **Automated checks run** (PR validation, CodeQL)
